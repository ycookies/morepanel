...
class InstallCommand extends Command
{
    ...
    // 通过sql创建后台应用面板 所需的数据表
    public function exeSqlFile(){
        $app = Helper::slug($namespace = $this->argument('name'));
        $contents = $this->getStub('sql');
        $new_sql = str_replace(
            ['DummyNamespace', 'DummyApp'],
            [$namespace, $app],
            $contents
        );
        $status = DB::unprepared($new_sql);
        $this->line('<info>Create New Panel Mysql Data:</info>'.$status);
    }

    public function createModelsFile(){
        $map = [
            'Administrator',
            'Menus',
            'Permissions',
        ...
        $app = Helper::slug($namespace = $this->argument('name'));
        foreach ($map as $fielss){
            $homeController = $this->directory.'/Models/'.$fielss.'.php';
            $contents = $this->getStub('/models/'.$fielss);
